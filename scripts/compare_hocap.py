...
Usage:
    ...
    PYTHONPATH=src python scripts/compare_hocap.py \
...
experiments/penetration_ablation/run.py with the matching --tag-suffix).
"""
...
def draw_primary_overlay(viewer, model, data, clips, hands, bimanual, vis,
    ...
    shown, matching demos/hocap.py behavior).
    """
    with viewer.lock():
...
def main():
    ...
    if missing:
        ...
        for p in missing:
            print(f"  {p}")
        print("\nGenerate with:")
        print(f"  PYTHONPATH=src python experiments/penetration_ablation/run.py "
              f"--clips {clip_id}")
    ...
    # HandRetargetConfig defaults (matches demos/hocap.py). Hard-coded here to
    ...
    # Same semantics as demos/hocap.py: overlays apply to PRIMARY hand only,
