...
Usage:
    python demos/hand/baseline/play_manus.py
    python demos/hand/baseline/play_manus.py --live
    python demos/hand/baseline/play_manus.py --collision
"""
...
from hand_retarget.mediapipe_io import load_pkl_sequence, preprocess_landmarks
...
def main():
    ...
    if not args.live:
        ...
        if qpos_cache is None:
            ...
            qpos_cache = np.zeros((total_frames, model.nq))
            t0 = time.time()
            for t in range(total_frames):
                qpos_cache[t] = retargeter.retarget(frames[t][f"{HAND_SIDE}_fingers"])
                if (t + 1) % 500 == 0:
    ...
    if not args.live:
        print(f"Keys:    SPACE=pause  LEFT=step/reverse  UP=source  DOWN=robot")
    print("=" * 50)
        ...
        while viewer.is_running():
            if args.live:
                ...
                if playback_state["paused"]:
                    ...
                    adv = int(dt / (avg_dt / args.speed))
                    if adv >= 1:
                        current_idx += playback_state["direction"] * adv
                        last_frame_time = now
                ...
                with viewer.lock():
                    ...
                    if vis_state["source"]:
                        for idx in MP_ALL_INDICES:
                            _add_sphere(viewer.user_scn, source_pts[idx], COL_SOURCE)
                    if vis_state["robot"]:
