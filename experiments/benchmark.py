...
class RetargetBenchmark:
    ...
    # MediaPipe DIP indices (for direction computation)
    DIP_INDICES = [3, 7, 11, 15, 19]
    # Finger names
    FINGER_NAMES = ["Thumb", "Index", "Middle", "Ring", "Pinky"]
    # All 10 pairwise tip combinations for inter-fingertip distance
    TIP_PAIRS = [(0,1), (0,2), (0,3), (0,4), (1,2), (1,3), (1,4), (2,3), (2,4), (3,4)]

    def __init__(self, urdf_path: str, hand_side: str = "left"):
    ...
    def _fk(self, q):
        """Run FK and return tip positions, DIP positions, palm position."""
    ...
    def evaluate(self, source_landmarks: np.ndarray, qpos_seq: np.ndarray,
                 fps: float = 156.0) -> dict:
        """
        Run all metrics.
        ...
        Args:
            ...
            fps: data frame rate (for jerk computation)
        ...
        for t in range(T):
            ...
            q = qpos_seq[t]
            violations = np.sum(q < self.model.lowerPositionLimit - 1e-6) + \
                         np.sum(q > self.model.upperPositionLimit + 1e-6)
        ...
        source_thumb_index_dists = np.array([
            np.linalg.norm(source_landmarks[t, 4] - source_landmarks[t, 8])
            for t in range(T)
        ]) * 1000  # mm
        pinch_mask = source_thumb_index_dists < 30  # near-pinch frames
        ...
        return {
            ...
            "pinch": {
                ...
                "per_pair": {name: float(inter_tip_errors[:, idx].mean())
                             for name, idx in zip(pinch_pair_names, pinch_pairs_idx)},
            },
            "num_frames": T,
        }
