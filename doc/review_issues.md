...
**20 个 bug:7 fully merged / 3 partial(完成了 safe subset)/ 1 rejected(负面结果)/ 9 open**:

| 状态 | 数量 | Bugs |
|---|---|---|
| ✅ 完全 merged | 7 | BUG-01 · 11 · 12 · 17 · 18 · 19(3/4 subset)· 20 |
| 🟡 Partial | 3 | BUG-09(alias 已清;**拆文件待做**)· BUG-14(part 1 + 2 已做;**FK-pass 合并待后续**)· BUG-15(buffer 已加;**active-set cache + broadphase 待做**) |
| ❌ Rejected | 1 | BUG-16(cache hit rate 太低 + 非热点,无性能收益) |
| ❌ Open | 9 | BUG-02 · 03 · 04 · 05 · 06 · 07 · 08 · 10 · 13 |

## 剩余 12 项(9 open + 3 partial 的剩余)分类
...
**小活基本清完** — BUG-14 part 2 已 merge(cbc7684),BUG-16 已 reject(无收益)。只剩 BUG-09 拆文件,被 background task 阻塞。
...
**状态**: merged (commit 28ff8da, 2026-04-23) — 消除 np.diag 构造 + broadcast row-scale;+3.6% fps;ULP 级 (7.77e-16 rad) 浮点重排序噪声,gate 远过
...
**状态**: partially merged — part 1 jacp buffer (3271db4); part 2 mp_idx→body_id 预解析 + `get_body_*_by_id` 方法 (cbc7684, Manus +3.3% fps);**part 3 FK-pass 合并待后续**(有 ULP 漂移风险,需 baseline 重建)
...
**状态**: partially merged (commit e31531d, 2026-04-23) — fromto + jacp buffer 预分配(HO-Cap byte-identical);active-set 缓存 / AABB 粗筛 / mj_collision broadphase 留后续(需 benchmark 驱动)
...
**状态**: **rejected (2026-04-23)** — 实测 simplices hash cache hit rate Manus 45% / HO-Cap 36%(远低于预想 95%:object 模式下 obj surface 样本浮动使 Delaunay 频繁变拓扑)。`get_adjacency_list` 不是热点(~百 μs),45% 命中 + bytes-hash 开销相互抵消,benchmark 无改善(Manus 100f 180→181ms)。**不 merge**,避免增加维护面。
